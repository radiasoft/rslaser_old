        ...
        for laser_index_i in np.arange(self.nslice):
            ...
            if not (
                ...
                re0_ey, re0_mesh_ey = srwutil.calc_int_from_wfr(
                    thisSlice.wfr, _pol=1, _int_type=5, _det=None, _fname="", _pr=False
                )
                im0_ey, im0_mesh_ey = srwutil.calc_int_from_wfr(
                    thisSlice.wfr, _pol=1, _int_type=6, _det=None, _fname="", _pr=False
                )
                ...
                wfr1 = srwlib.SRWLWfr(
                    ...
                    _ne=1,
                    _xStart=np.min(thisSlice.initial_laser_xy.x),
                    _xFin=np.max(thisSlice.initial_laser_xy.x),
                    _nx=len(thisSlice.initial_laser_xy.x),
                    _yStart=np.min(thisSlice.initial_laser_xy.y),
                    _yFin=np.max(thisSlice.initial_laser_xy.y),
                    _ny=len(thisSlice.initial_laser_xy.y),
                    _zStart=0.0,
                    _partBeam=None,
                )
                thisSlice.wfr = wfr1
...
class LaserPulseSlice(ValidatorBase):
    ...
    def _wavefront(self, params, files):
        if files:
            ...
            ex = array.array("f", ex_numpy.tolist())
            ey = array.array("f", len(ex) * [0.0])  # Assume ey is zero
            self.wfr = srwlib.SRWLWfr(
                _arEx=ex,
                _arEy=ey,
                _typeE="f",
                _eStart=self.photon_e_ev,
                _eFin=self.photon_e_ev,
                _ne=1,
                _xStart=x_min,
                _xFin=x_max,
                _nx=nx,
